...
import {FooterButton } from "../../../components/SharedButton";
...
const AccademicDepartment = () => {
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useGetAllAccademicDepartmentQuery(params);
  const navigate = useNavigate();
  const { isMobile } = useResponsive();
  //console.log(academicDepartmentData);

  const facultyFilters = academicDepartmentData?.data
    ?.map((dep: TAcademicDepartment) => ({
  ...
  const tableData: TTableData[] =
    academicDepartmentData?.data?.map((department: TAcademicDepartment) => ({
      key: department._id,
  ...
  const onChange: TableProps<TTableData>["onChange"] = (
    ...
    if (extra.action === "filter") {
      ...
      filters.name?.forEach((faculty_id) => {
        //in here filters.jekhane filter option add korchi sei column er name dite hobe, jemon set korchi name column e
        queryParams.push({ name: "academicFaculty", value: faculty_id });
      }); //ekhane database e je field onujayi data khujchi seta onujayi dite hobe jemon 'academicFaculty' name ace database e
      ...
      <Flex vertical justify="space-between" style={{ minHeight: "80vh" }}>
        <Card
          style={{
            ...
            border: ".7px solid #d7dce5ff",
            marginTop: "20px",
          }}
        >
          <Table<TTableData>
            columns={columns}
            style={{ width: "100%", border: ".5px solid #d7dce5ff" }}
            loading={isFetching}
        ...
        <div>
          <FooterButton
            okButton="Create more"
            cancleButton={"Cancel"}
            handleOk={handleOkButton}
            handleCancle={handleCancleButton}
          ></FooterButton>
        </div>
      </Flex>
    </Flex>
